...
# Paths
model_path = os.path.join(base_dir, "gender_recognition_model_28.keras")
...
# Check current shape of test_images
print(f"Original shape of test_images: {test_images.shape}")

# Resize or reshape to match model input
if test_images.shape[1:] != (28, 28, 1):  # If not already (28, 28, 1)
    import cv2
    test_images = test_images.squeeze(1)
    test_images = test_images.reshape(-1, 28, 28, 1)

print(f"Reshaped test_images to: {test_images.shape}")
...
assert predicted_labels.shape[0] == test_labels.shape[0], \
...
false_negatives_images = test_images[false_negatives_idx]
...
for i, idx in enumerate(false_negatives_idx):
    img = false_negatives_images[i].reshape(28, 28)  # Adjust shape if necessary
    plt.imsave(os.path.join(false_negatives_dir, f"false_negative_{idx}.png"), img, cmap='gray')
